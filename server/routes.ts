...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Customer API: Get sprocket alternatives based on FCW Group and RCW Group
  app.get("/api/customer/motorcycles/:recid/sprocket-alternatives", async (req, res) => {
    try {
      const recid = parseInt(req.params.recid);
      if (isNaN(recid)) {
        return res.status(400).json({ message: "Invalid motorcycle ID" });
      ...
      // Get the motorcycle to access fcwgroup and rcwgroup values
      const motorcycle = await storage.getMotorcycle(recid);
      if (!motorcycle) {
        return res.status(404).json({ message: "Motorcycle not found" });
      }

      // Fetch all Shopify products to search for alternatives
      const allProducts = await storage.getShopifyProducts();
      
      const alternatives = {
        frontSprocketAlternatives: [] as any[],
        rearSprocketAlternatives: [] as any[]
      };

      // FCW Group alternatives (Front Chain Wheel)
      if (motorcycle.fcwgroup) {
        const fcwGroupValue = motorcycle.fcwgroup.trim();
        console.log(`🔧 FCW Group value: ${fcwGroupValue}`);
        
        // If fcwgroup contains the product title, find the product by title and use its SKU variants as alternatives
        const fcwGroupProduct = allProducts.find(p => p.title === fcwGroupValue);
        
        if (fcwGroupProduct && fcwGroupProduct.variants) {
          const variants = JSON.parse(fcwGroupProduct.variants);
          console.log(`🔧 Found FCW Group product: ${fcwGroupProduct.title} with ${variants.length} variants`);
          
          // Add each variant as an alternative (excluding the OE one)
          for (const variant of variants) {
            if (variant.sku && variant.sku !== motorcycle.oe_fcw) {
              alternatives.frontSprocketAlternatives.push({
                id: fcwGroupProduct.id,
                title: `${fcwGroupProduct.title} - ${variant.title}`,
                sku: variant.sku,
                price: variant.price,
      ...
      // RCW Group alternatives (Rear Chain Wheel)
      if (motorcycle.rcwgroup) {
        const rcwGroupValue = motorcycle.rcwgroup.trim();
        console.log(`🔧 RCW Group value: ${rcwGroupValue}`);
        
        // If rcwgroup contains the product title, find the product by title and use its SKU variants as alternatives
        const rcwGroupProduct = allProducts.find(p => p.title === rcwGroupValue);
        
        if (rcwGroupProduct && rcwGroupProduct.variants) {
          const variants = JSON.parse(rcwGroupProduct.variants);
          console.log(`🔧 Found RCW Group product: ${rcwGroupProduct.title} with ${variants.length} variants`);
          
          // Add each variant as an alternative (excluding the OE one)
          for (const variant of variants) {
            if (variant.sku && variant.sku !== motorcycle.oe_rcw) {
              alternatives.rearSprocketAlternatives.push({
                id: rcwGroupProduct.id,
                title: `${rcwGroupProduct.title} - ${variant.title}`,
                sku: variant.sku,
                price: variant.price,
      ...
      console.log(`✅ Found ${alternatives.frontSprocketAlternatives.length} FCW alternatives and ${alternatives.rearSprocketAlternatives.length} RCW alternatives for RECID ${recid}`);
      
      res.json(alternatives);
    } catch (error) {
      console.error('Sprocket alternatives error:', error);
      res.status(500).json({ message: "Failed to fetch sprocket alternatives" });
    }
  });
  ...
  // App proxy route for sprocket alternatives (used by Theme App Extension)
  app.get("/api/proxy/api/customer/motorcycles/:recid/sprocket-alternatives", appProxySecurityMiddleware, async (req, res) => {
    try {
      const recid = parseInt(req.params.recid);
      if (isNaN(recid)) {
        return res.status(400).json({ message: "Invalid motorcycle ID" });
      ...
      if (motorcycle.fcwgroup) {
        const fcwGroupValue = motorcycle.fcwgroup.trim();
        console.log(`🔧 PROXY: FCW Group value: ${fcwGroupValue}`);
        ...
        if (fcwGroupProduct && fcwGroupProduct.variants) {
          const variants = JSON.parse(fcwGroupProduct.variants);
          console.log(`🔧 PROXY: Found FCW Group product: ${fcwGroupProduct.title} with ${variants.length} variants`);
      ...
      if (motorcycle.rcwgroup) {
        const rcwGroupValue = motorcycle.rcwgroup.trim();
        console.log(`🔧 PROXY: RCW Group value: ${rcwGroupValue}`);
        ...
        if (rcwGroupProduct && rcwGroupProduct.variants) {
          const variants = JSON.parse(rcwGroupProduct.variants);
          console.log(`🔧 PROXY: Found RCW Group product: ${rcwGroupProduct.title} with ${variants.length} variants`);
      ...
      console.log(`✅ PROXY: Found ${alternatives.frontSprocketAlternatives.length} FCW alternatives and ${alternatives.rearSprocketAlternatives.length} RCW alternatives for RECID ${recid}`);
      
      res.json(alternatives);
    } catch (error) {
      console.error("Error fetching sprocket alternatives via app proxy:", error);
      res.status(500).json({ error: "Failed to fetch sprocket alternatives" });
    }
  });
